    ...
    def forward(self, x, need_fp=False, use_corr=False):
        ...
        if need_fp:
            ...
            if use_corr:
                ...
                dict_return['f1_2nd_diff'] = corr_out_dict['f1_2nd_diff']
                dict_return['f2_2nd_diff'] = corr_out_dict['f2_2nd_diff']
                dict_return['corr_map'] = corr_out_dict['corr_map']
                dict_return['corr_map_ori'] = corr_out_dict['corr_map_ori']
                dict_return['corr_map_ori_1st_diff'] = corr_out_dict['corr_map_ori_1st_diff']
...
class Corr(nn.Module):
    ...
    def forward(self, feature_in, out):
        ...
        f1_1st_diff = f1[:, 1:, :] - f1[:, :-1, :]
        f2_1st_diff = f2[:, 1:, :] - f2[:, :-1, :]

        f1_2nd_diff = f1_1st_diff[:, 1:, :] - f1_1st_diff[:, :-1, :]
        f2_2nd_diff = f2_1st_diff[:, 1:, :] - f2_1st_diff[:, :-1, :]


        out_temp = rearrange(out, 'n c h w -> n c (h w)')
        corr_map = torch.matmul(f1.transpose(1, 2), f2) / torch.sqrt(torch.tensor(f1.shape[1]).float())
        corr_map_1st_diff = torch.matmul(f1_1st_diff.transpose(1, 2), f2_1st_diff) / torch.sqrt(torch.tensor(f1_1st_diff.shape[1]).float())
        
        corr_map = F.softmax(corr_map + corr_map_1st_diff, dim=-1)
        corr_map_sample = self.sample(corr_map.detach(), h_in, w_in)

        corr_map_1st_diff = torch.matmul(f1_1st_diff.transpose(1, 2), f2_1st_diff) / torch.sqrt(torch.tensor(f1_1st_diff.shape[1]).float())
        corr_map_1st_diff = F.softmax(corr_map_1st_diff, dim=-1)
        corr_map_sample_1st_diff = self.sample(corr_map_1st_diff.detach(), h_in, w_in)

        dict_return['f1_2nd_diff'] = F.softmax(f1_2nd_diff, dim=-1)
        dict_return['f2_2nd_diff'] = F.softmax(f2_2nd_diff, dim=-1)

        dict_return['corr_map'] = self.normalize_corr_map(corr_map_sample, h_in, w_in, h_out, w_out)

        dict_return['corr_map_ori'] = torch.matmul(F.softmax(f1, dim=1).transpose(1, 2), F.softmax(f2, dim=1)) / torch.sqrt(torch.tensor(f1.shape[1]).float())
        dict_return['corr_map_ori_1st_diff'] = torch.matmul(F.softmax(f1_1st_diff, dim=1).transpose(1, 2), F.softmax(f2_1st_diff, dim=1)) / torch.sqrt(torch.tensor(f1_1st_diff.shape[1]).float())
